...
Uses SentenceTransformers for local embedding generation and FAISS
for semantic similarity search. Supports building, saving, loading,
and querying a vector index.
"""
...
from utils.config import EMBEDDING_MODEL, EMBEDDING_DIMENSION, FAISS_DIR
...
class EmbeddingEngine:
    """
    Manages embedding generation via SentenceTransformers
    and similarity search via FAISS.
    """
    ...
    def __init__(self, model_name: str = EMBEDDING_MODEL):
        ...
        self.metadata: list[dict] = []  # Store metadata alongside vectors
    ...
    def build_profile_embedding(self, profile: dict) -> np.ndarray:
        ...
        Combines positioning statement, value proposition, key features,
        and marketing style into a single embedding vector.
        ...
        # Industry and target customer are critical for relevance — weight them higher
        ...
        if profile.get("products_services"):
            ...
            if isinstance(products, list):
                texts.append(f"Products and services: {', '.join(str(p) for p in products)}")
            else:
    ...
    def save_index(self, name: str = "default") -> str:
        ...
        save_dir = FAISS_DIR / name
        save_dir.mkdir(parents=True, exist_ok=True)

        index_path = str(save_dir / "index.faiss")
        meta_path = str(save_dir / "metadata.pkl")

        faiss.write_index(self.index, index_path)
        with open(meta_path, "wb") as f:
            pickle.dump(self.metadata, f)

        logger.info("FAISS index saved: %s (%d vectors)", index_path, self.index.ntotal)
        return str(save_dir)
    ...
    def load_index(self, name: str = "default") -> bool:
        ...
        logger.info("FAISS index loaded: %s (%d vectors)", index_path, self.index.ntotal)
        return True
