...
1. LLM-based discovery (Groq Llama) — asks the LLM to identify competitors
   based on the business profile. Fast and reliable.
2. DuckDuckGo SERP search (fallback) — used to supplement LLM results
   when available, with frequency and semantic similarity scoring.
"""
...
from utils.config import (
    GROQ_API_KEY, GROQ_MODEL, MAX_COMPETITORS,
    MAX_SEARCH_RESULTS, SEARCH_RATE_LIMIT_DELAY,
)
from utils.helpers import (
    extract_domain, safe_json_parse, retry_with_backoff,
)
...
def _detect_country(source_url: str, source_domain: str, profile: dict) -> str:
    ...
    if not country:
        ...
        if isinstance(geo, str) and geo and geo.lower() not in ("global", "worldwide", "not detected", "n/a", ""):
            country = geo
...
def discover_competitors(
    ...
    """
    Discover and rank competitors using LLM-based discovery
    with optional DuckDuckGo search supplementation.
    ...
    # ── Primary: LLM-based discovery ──────────────────────────────────────
    logger.info("Discovering %s competitors via LLM...", scope)
    llm_competitors = _discover_via_llm(
        business_profile, source_url, source_domain, max_competitors, scope,
    )
    logger.info("LLM discovered %d %s competitors", len(llm_competitors), scope)

    # ── Secondary: DuckDuckGo search (best-effort) ─────────────────────────
    # Skip DDG if LLM already gave us enough candidates
    if len(llm_competitors) >= max_competitors:
        logger.info(
            "LLM returned %d candidates (>= %d needed), skipping DDG for %s",
            len(llm_competitors), max_competitors, scope,
        )
        ddg_competitors = []
    else:
        ddg_competitors = _discover_via_duckduckgo(
            business_profile, source_domain, scope,
        )
        logger.info("DuckDuckGo discovered %d additional %s candidates", len(ddg_competitors), scope)

    # ── Merge results ──────────────────────────────────────────────────────
    merged = _merge_results(llm_competitors, ddg_competitors, source_domain)

    # ── Semantic similarity scoring ────────────────────────────────────────
    if profile_embedding is not None and embedding_engine is not None:
        merged = _score_semantic_similarity(
            merged, profile_embedding, embedding_engine,
        )
...
def _discover_via_llm(
    ...
    """Use Groq Llama to identify competitors from business profile."""
    try:
...
def _discover_via_duckduckgo(
    ...
    """Best-effort DuckDuckGo search for additional competitors."""
    try:
    ...
    products_str = ", ".join(products[:3]) if isinstance(products, list) else str(products)

    # Scope-specific query modifiers
    if scope == "local":
        geo_terms = ["India", "Indian brands"]
    else:
        geo_terms = ["global", "international"]
    geo_suffix = " ".join(geo_terms[:1])  # e.g. "India" or "global"

    # Build queries — limit to 2 highest-signal queries to avoid DDG rate limits
    queries = []
    if brand_name:
        queries.append(f"{brand_name} competitors {geo_suffix}")
    if industry and target_customer:
        queries.append(f"Best {industry} {geo_suffix} for {target_customer}")
    elif industry:
        queries.append(f"Best {industry} companies {geo_suffix}")

    # Cap at 2 queries per scope to reduce DDG load
    queries = queries[:2]

    all_results = []
    # Reuse a single DDGS session for all queries to avoid bot detection
    try:
        ddgs = DDGS()
    except Exception as e:
    ...
    for query in queries:
        try:
            ...
            logger.debug("DDG query '%s' returned %d results", query, len(results))
        except Exception as e:
            err_str = str(e).lower()
            if "ratelimit" in err_str or "202" in err_str or "429" in err_str:
                logger.warning(
                    "DDG rate limited on '%s': %s — aborting remaining queries",
                    query, e,
                )
                break  # stop all DDG queries for this scope
            logger.warning("DDG search failed for '%s': %s", query, e)

        time.sleep(SEARCH_RATE_LIMIT_DELAY)

    # Aggregate by domain
    return _aggregate_domains(all_results, source_domain)
...
def _aggregate_domains(
    ...
    excluded_domains = {
        ...
        "techcrunch.com", "crunchbase.com", "g2.com",
        "capterra.com", "trustpilot.com", "glassdoor.com",
        "amazon.com", "amazon.in", "google.com", "bing.com",
        "flipkart.com", "myntra.com", "ajio.com", "snapdeal.com",
        "nykaa.com", "meesho.com", "ebay.com", "etsy.com",
...
def _merge_results(
    ...
    for c in llm_candidates:
        ...
        if domain and domain != source_domain and base_domain not in seen_base_domains and name_key not in seen_names:
            seen_base_domains.add(base_domain)
            if name_key:
                seen_names.add(name_key)
            merged.append(c)
...
def validate_competitor_relevance(
    ...
    try:
        ...
        prompt = RELEVANCE_VALIDATION_PROMPT.format(
            ...
            source_products=", ".join(source_products) if isinstance(source_products, list) else str(source_products),
            ...
            candidate_products=", ".join(candidate_products) if isinstance(candidate_products, list) else str(candidate_products),
        ...
        logger.warning("Could not parse relevance response, assuming relevant: %s", text[:200])
