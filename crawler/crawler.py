...
Features:
    ...
    - Max crawl depth = 3, Max pages = 50
...
from utils.config import (
    MAX_CRAWL_DEPTH, MAX_PAGES, CRAWL_TIMEOUT_MS,
    SCROLL_PAUSE_MS, MAX_SCROLLS, SCREENSHOTS_DIR,
)
from utils.helpers import (
    normalize_url, extract_domain, is_same_domain,
    resolve_url, content_hash, is_valid_url, is_crawlable_url,
)
...
class AdaptiveCrawler:
        ...
        async with async_playwright() as pw:
            ...
            while queue and len(self.pages) < self.max_pages:
                ...
                page_data, links = await self._crawl_page(
                    context, current_url, depth, base_domain,
                )
                ...
                logger.info(
                    ...
                    len(self.pages), self.max_pages, depth, current_url,
                )
    ...
    async def _crawl_page(
        ...
        try:
            # Progressive fallback: try domcontentloaded first, then commit
            response = None
            ...
            if response.status >= 400:
                logger.warning(
                    "Failed to load %s (status: %s)", url, response.status,
                )
                return None, []

            # Check for domain redirects (e.g., expired domains, parking pages)
            final_url = page.url
            ...
            parking_indicators = [
                "domain for sale", "parked", "buy this domain",
                "this domain is for sale", "domain expired",
                "godaddy", "hugedomains", "sedo domain parking",
