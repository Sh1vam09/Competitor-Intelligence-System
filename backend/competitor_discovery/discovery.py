...
from backend.utils.llm_wrapper import call_llm_with_fallback
...
def _normalize_tracxn_brand_key(text: str) -> str:
    """Normalize a brand or Tracxn slug for loose exact matching."""
    return re.sub(r"[^a-z0-9]+", "", (text or "").lower())


def _brand_aliases_for_tracxn(brand_name: str) -> list[str]:
    """Generate search/match aliases, stripping legal suffixes like Pvt Ltd."""
    cleaned = re.sub(r"\s+", " ", (brand_name or "").replace("&", " and ")).strip()
    ...
    while core_tokens and core_tokens[-1].lower() in _LEGAL_ENTITY_SUFFIX_TOKENS:
        core_tokens.pop()
    ...
    for alias in (
        " ".join(core_tokens),
        cleaned,
        re.sub(r"\bpvt\b", "private", cleaned, flags=re.I),
        re.sub(r"\bprivate\b", "pvt", cleaned, flags=re.I),
    ):
...
def _tracxn_brand_keys(brand_name: str) -> set[str]:
    """Return normalized keys accepted as the same brand on Tracxn."""
    keys = {
        key
        for key in (
            _normalize_tracxn_brand_key(alias)
            for alias in _brand_aliases_for_tracxn(brand_name)
        )
        if key
    }
    return keys
...
def _extract_tracxn_company_slug(href: str) -> str:
    """Extract the Tracxn company/legal-entity slug from a URL."""
    slug_match = re.search(r"/d/(?:companies|legal-entities/[^/]+)/([^/]+)/", href)
    if not slug_match:
        return ""
    return slug_match.group(1)


def _score_tracxn_brand_result(result: dict, brand_name: str) -> int:
    """Score how well a Tracxn search result matches the requested brand."""
    href = (result.get("href") or "").strip().lower()
    title = (result.get("title") or "").strip().lower()
    content = (result.get("content") or result.get("body") or "").strip().lower()
    ...
    brand_keys = _tracxn_brand_keys(brand_name)
    if not brand_keys:
        return 0

    normalized_title = _normalize_tracxn_brand_key(title)
    normalized_content = _normalize_tracxn_brand_key(content)
    slug_key = _normalize_tracxn_brand_key(_extract_tracxn_company_slug(href))

    score = 0
    if slug_key and slug_key in brand_keys:
        score += 80
    elif slug_key and any(slug_key in key or key in slug_key for key in brand_keys):
        score += 45

    if any(key and key in normalized_title for key in brand_keys):
        score += 20
    if any(key and key in normalized_content for key in brand_keys):
        score += 10

    if "/d/companies/" in href:
        score += 30
    elif "/d/legal-entities/" in href:
        score -= 30
...
def _matches_tracxn_brand_result(result: dict, brand_name: str) -> bool:
    ...
    return _score_tracxn_brand_result(result, brand_name) > 0


def _rank_tracxn_results(results: list[dict], brand_name: str) -> list[dict]:
    """Return matching Tracxn results, preferring company pages over legal pages."""
    ...
    for index, result in enumerate(results):
        ...
        if (
            not href
            or href_lower in seen_hrefs
            or "tracxn.com" not in href_lower
            or "/login" in href_lower
            or "/signup" in href_lower
        ):
            continue

        score = _score_tracxn_brand_result(result, brand_name)
        if score <= 0:
            logger.info(
        ...
        seen_hrefs.add(href_lower)
        ranked.append((score, index, result))
        logger.info(
            "Tracxn candidate for %s scored %d: %s",
            brand_name,
            score,
            href,
        )
...
def discover_competitors(
    ...
    all_candidates = _filter_candidate_pool(all_candidates, source_domain)
    if scope == "local" and tracxn_competitors:
        logger.info(
            "Skipping LLM validation for %d trusted Tracxn local candidates",
            len(tracxn_competitors),
        )
        llm_validated = []
...
def _discover_via_llm(
    ...
    try:
        ...
        local_context = ""
        if scope == "global" and profile.get("local_competitor_context"):
            local_context = (
                "\nTRUSTED LOCAL COMPETITOR BENCHMARK:\n"
                f"{profile.get('local_competitor_context')}\n"
                "Use this benchmark to identify global companies with the same "
                "business category, customer problem, and product pattern."
            )
        ...
        prompt = COMPETITOR_DISCOVERY_PROMPT.format(
            ...
            local_competitor_context=local_context,
...
def _discover_via_tracxn(
    ...
    brand_aliases = _brand_aliases_for_tracxn(brand_name)
    if not brand_aliases:
        logger.warning("No usable brand aliases found for Tracxn search: %s", brand_name)
        return []

    # Search for Tracxn pages using the clean brand first. Legal suffixes such
    # as "Pvt. Ltd" often point search at legal-entity pages instead of the
    # product/company profile that contains competitors.
    tracxn_queries = []
    for alias in brand_aliases[:3]:
        tracxn_queries.extend(
            [
                f"tracxn {alias}",
                f"site:tracxn.com/d/companies {alias}",
            ]
        )
    tracxn_queries = list(dict.fromkeys(tracxn_queries))
    logger.info(
        "Searching Tracxn for %s (aliases: %s, queries: %s)",
        brand_name,
        brand_aliases,
        tracxn_queries,
    )
    ...
    try:
        ...
        for query in tracxn_queries:
            results = _tavily_search(query, max_results=10)
            for result in results:
                href = (result.get("href") or "").strip()
                href_key = href.lower()
                if not href or href_key in seen_result_urls:
                    continue
                seen_result_urls.add(href_key)
                search_results.append(result)
            time.sleep(SEARCH_RATE_LIMIT_DELAY)

        tracxn_results = _rank_tracxn_results(search_results, brand_name)
        if not tracxn_results:
            logger.info("No Tracxn page found for %s", brand_name)
            return []

        all_competitors: list[dict] = []
        seen_competitors: set[str] = set()
        for result in tracxn_results[:5]:
            tracxn_url = result.get("href", "")
            logger.info("Trying Tracxn page for %s: %s", brand_name, tracxn_url)
            ...
            competitors = [
                ...
                if c.get("domain")
                and not _is_excluded_candidate_domain(
                    c.get("domain", ""), c.get("url", "")
                )
            ]

            if not competitors:
                logger.info(
                    "Tracxn page yielded no usable competitors for %s: %s",
                    brand_name,
                    tracxn_url,
                )
                continue
            ...
            for comp in competitors:
                key = (comp.get("domain") or comp.get("name") or "").strip().lower()
                if key and key not in seen_competitors:
                    seen_competitors.add(key)
                    all_competitors.append(comp)

            logger.info(
                "Tracxn page yielded %d usable competitors for %s: %s",
                len(competitors),
                brand_name,
                tracxn_url,
            )
        ...
        if all_competitors:
            logger.info(
                ...
                brand_name,
                [c.get("name", "") for c in all_competitors],
            )
