...
def generate_report(
    ...
    elements.append(Paragraph(
        ...
        ParagraphStyle("CompanyTitle", parent=styles["Title"], fontSize=18,
                       textColor=colors.HexColor("#0f3460"), alignment=TA_CENTER),
    ))
    elements.append(Spacer(1, 5))
    elements.append(Paragraph(
        f"<i>{_escape(company_url)}</i>",
        ParagraphStyle("URLStyle", parent=styles["Normal"], fontSize=10,
    ...
    elements.append(Spacer(1, 10))
    elements.append(HRFlowable(width="80%", color=colors.HexColor("#0f3460"),
                               thickness=2, spaceAfter=20))
    ...
    try:
        ...
        elements.append(Paragraph("Business profile data could not be rendered.", styles["BodyJustified"]))
    elements.append(Spacer(1, 10))

    # ── 3. DOM & Structural Analysis ───────────────────────────────────────
    elements.append(Paragraph("3. Website Structural Analysis", styles["SectionHeader"]))
    try:
        elements.extend(_build_dom_section(dom_features))
    except Exception as e:
        logger.warning("Report section 'DOM Analysis' failed: %s", e)
        elements.append(Paragraph("DOM analysis data could not be rendered.", styles["BodyJustified"]))
    elements.append(Spacer(1, 10))
    ...
    try:
        ...
        elements.append(Paragraph("Visual analysis data could not be rendered.", styles["BodyJustified"]))
    elements.append(PageBreak())
    ...
    if _local:
        try:
            ...
            elements.append(Paragraph("Local competitor list could not be rendered.", styles["BodyJustified"]))
    else:
        elements.append(Paragraph("No local (Indian) competitors were discovered.", styles["BodyJustified"]))
    elements.append(Spacer(1, 10))
    ...
    if _global:
        try:
            ...
            elements.append(Paragraph("Global competitor list could not be rendered.", styles["BodyJustified"]))
    else:
        elements.append(Paragraph("No global competitors were discovered.", styles["BodyJustified"]))
    elements.append(Spacer(1, 10))

    # ── 6A. Local Competitor Deep Profiles ─────────────────────────────────
    if _local:
        elements.append(Paragraph("6A. Local Competitor Deep Profiles", styles["SectionHeader"]))
        try:
            elements.extend(_build_competitor_profiles(_local))
        except Exception as e:
            logger.warning("Report section 'Local Competitor Profiles' failed: %s", e)
            elements.append(Paragraph("Local competitor profiles could not be rendered.", styles["BodyJustified"]))
        elements.append(Spacer(1, 10))

    # ── 6B. Global Competitor Deep Profiles ────────────────────────────────
    if _global:
        elements.append(Paragraph("6B. Global Competitor Deep Profiles", styles["SectionHeader"]))
        try:
            elements.extend(_build_competitor_profiles(_global))
        except Exception as e:
            logger.warning("Report section 'Global Competitor Profiles' failed: %s", e)
            elements.append(Paragraph("Global competitor profiles could not be rendered.", styles["BodyJustified"]))
    elements.append(PageBreak())

    # ── 7. Side-by-Side Comparison ─────────────────────────────────────────
    elements.append(Paragraph("7. Side-by-Side Comparison", styles["SectionHeader"]))
    try:
        elements.extend(_build_comparison_section(comparison, business_profile, competitors))
    except Exception as e:
        logger.warning("Report section 'Comparison' failed: %s", e)
        elements.append(Paragraph("Comparison data could not be rendered.", styles["BodyJustified"]))
    elements.append(Spacer(1, 10))

    # ── 8. Strategic Insights ──────────────────────────────────────────────
    elements.append(Paragraph("8. Strategic Threat Assessment", styles["SectionHeader"]))
    try:
        elements.extend(_build_threats_section(comparison))
    except Exception as e:
        logger.warning("Report section 'Threats' failed: %s", e)
        elements.append(Paragraph("Threat assessment could not be rendered.", styles["BodyJustified"]))
    elements.append(Spacer(1, 10))
    ...
    try:
        ...
        elements.append(Paragraph("Opportunities data could not be rendered.", styles["BodyJustified"]))
    elements.append(Spacer(1, 10))
    ...
    try:
        ...
        elements.append(Paragraph("Recommendations could not be rendered.", styles["BodyJustified"]))
...
def _build_profile_section(profile: dict) -> list:
    ...
    # Array fields
    for label, key in [("Products/Services", "products_services"),
...
def _build_visual_section(visual_profile: dict) -> list:
    ...
    text_fields = ["visual_brand_personality", "color_psychology", "emotional_tone_visual"]
    for field in text_fields:
        value = visual_profile.get(field, "N/A")
        label = field.replace("_", " ").title()
        elements.append(Paragraph(f"<b>{label}:</b> {_escape(str(value))}", styles["BodyJustified"]))
...
def _build_competitor_list(competitors: list[dict]) -> list:
    ...
    if len(rows) > 1:
        ...
        elements.append(Paragraph("No competitors discovered.", styles["BodyJustified"]))
...
def _build_competitor_profiles(competitors: list[dict]) -> list:
    ...
    for i, comp in enumerate(competitors, 1):
        ...
        if profile:
            ...
            elements.append(Paragraph("Profile data not available.", styles["BodyJustified"]))
...
def _build_comparison_section(comparison: dict, profile: dict, competitors: list[dict]) -> list:
    """Build side-by-side comparison section."""
    elements = []

    for field in ["positioning_comparison", "pricing_comparison", "brand_personality_differences",
    ...
    if isinstance(gap, dict):
        ...
        for sub_key in ["input_company_unique_features", "competitor_unique_features",
                        "common_features", "emerging_features"]:
            items = gap.get(sub_key, [])
            label = sub_key.replace("_", " ").title()
            elements.append(Paragraph(f"<i>{label}:</i>", styles["Normal"]))
            if isinstance(items, list) and items:
                for item in items:
                    elements.append(Paragraph(f"  • {_escape(str(item))}", styles["Normal"]))
            else:
        ...
        for threat in threats:
            if isinstance(threat, dict):
                rows.append([
                    ...
                    Paragraph(_wrap(str(threat.get("threat_reasoning", "N/A")), 100), styles["Normal"]),
                    Paragraph(_wrap(str(threat.get("defensive_recommendation", "N/A")), 100), styles["Normal"]),
                ])
        table = Table(rows, colWidths=[80, 65, 200, 155])
        table.setStyle(TABLE_STYLE)
        elements.append(table)
    else:
        elements.append(Paragraph("No threat assessment available.", styles["BodyJustified"]))
    ...
    if isinstance(opportunities, list) and opportunities:
        ...
        for opp in opportunities:
            if isinstance(opp, dict):
                rows.append([
                    Paragraph(_wrap(str(opp.get("opportunity", "N/A")), 80), styles["Normal"]),
                    Paragraph(_wrap(str(opp.get("rationale", "N/A")), 80), styles["Normal"]),
        ...
        elements.append(Paragraph("No opportunities identified.", styles["BodyJustified"]))
    ...
    if isinstance(recommendations, list) and recommendations:
        ...
        for rec in recommendations:
            if isinstance(rec, dict):
                rows.append([
                    Paragraph(_wrap(str(rec.get("recommendation", "N/A")), 80), styles["Normal"]),
                    _escape(str(rec.get("priority", "N/A"))),
                    Paragraph(_wrap(str(rec.get("expected_impact", "N/A")), 70), styles["Normal"]),
                    Paragraph(_wrap(str(rec.get("implementation_notes", "N/A")), 70), styles["Normal"]),
                ])
        table = Table(rows, colWidths=[170, 70, 130, 130])
        table.setStyle(TABLE_STYLE)
        elements.append(table)
    else:
        elements.append(Paragraph("No recommendations available.", styles["BodyJustified"]))
