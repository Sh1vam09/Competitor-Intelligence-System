...
### Problem 1: DDG Returning Irrelevant Results

**Issue:** DuckDuckGo search returned news sites, Q&A sites (zhihu, stackexchange), and generic information domains (skincare.com, beauty.com) instead of actual competitor companies.

**Root Cause:** The original approach relied on LLM-generated search queries, which were not specific enough to filter out non-company websites.

**Solution Implemented:**
- Added `_is_relevant_domain()` function to pre-filter obviously bad domains
- Created domain rejection patterns for news sites, Q&A platforms, blog sites, and generic one-word domains
- Updated LLM validation prompt with explicit rejection rules for specific site types
...
**Root Cause:** Tracxn pages load competitor lists dynamically via JavaScript, which the basic crawler cannot render.

**Solution Implemented:**
- Updated regex patterns to match multiple formats:
  - "Competitors: BrandA, BrandB, BrandC"
  - "Alternatives: X, Y, Z"
  - "Similar companies: Foxtale & 1277 more"
  - "companies like Foxtale, Deconstruct"
- Added pattern for "companies like [X], [Y], and [Z]" format commonly used on Tracxn
...
**Root Cause:** 
1. The LLM was returning JSON in a wrapper format (`{"competitors": [...]}`) instead of a plain array
2. The validation prompt was not explicit enough about rejection criteria

**Solution Implemented:**
- Updated `_validate_via_llm()` to handle both list and object JSON formats
- Added explicit rejection rules in the prompt for news, Q&A, blog, and generic sites
- Made validation stricter about requiring same industry, same niche products
...
**Root Cause:** Multiple API calls for validation and discovery were consuming tokens quickly.

**Solution Implemented:**
- Added rate limit error handling with graceful fallback
- When LLM fails, code falls back to DDG results
- Validation checks if valid_count < 3 to trigger LLM fallback discovery
